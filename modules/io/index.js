...
function onConnection(socket) {
    ...
    socket.on('verifyReq', async function onVerification(phone, type) {
        ...
        if (type === 'sms') {            
            if (smsAuth.sendSMSCode(phone, code)) socket.emit('verifyConfirm', 'Verification sms was sent', phone, null); 
            ...
            socket.emit('verifyConfirm', 'Call ' + result.prettyPhone + ' to verify', phone, null); 
        }
    });
    ...
    socket.on('restoreSession', socket.restoreSession.bind(this));
