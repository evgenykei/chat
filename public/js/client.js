...
function sendJoinReq(tryRoomID, tryCode){
  //load up the globals with our given data
  if (tryCode === "" || tryCode.length < 4 || tryRoomID === "" || tryRoomID.length < 2) {
    //we have a problem
    $("#connect-status").append("<li>Please verify your number and enter room name longer than 2 characters.</li>");
    ...
    myRoomID = tryRoomID;
    password = myRoomID + "temporaryPassword";

    //make it so
    $("#connect-status").append("<li>Sending join request</li>");
    socket.emit("joinReq", CryptoJS.SHA1(myRoomID).toString(), CryptoJS.SHA1(password).toString(), tryCode.toString());
  }
}
...
//vars for room data
var myRoomID = password = phone = null;
...
$(document).ready(function () {
  ...
  function createMenu(buttonMenu) {
    ...
    buttonMenu.forEach(function(button) {
      menu.append('<button id="' + button.action + '" class="menu-button btn-lg">' + button.title + '</button>')
    });
  ...
  socket.on('buttonMenu', function(buttonMenu) {
    menuRoot = buttonMenu;
    createMenu(buttonMenu);
  });
  ...
  socket.on("chat", function (payload) {
    ...
    else if (msgCore.indexOf(phone) > -1) {
      //we're mentioned
    ...
    postChat("<div class=\"message " + msgOwner + "\" id=\"message-" + messageCount + "\"><span class=\"message-metadata\"><span class=\"message-name\">" + sanitizeToHTMLSafe(msgPhone) + "</span><br />" + getHTMLStamp() + "</strong></span><span class=\"message-body\"> " + msgCore + "</span></div>", (msgCore.indexOf(phone) > -1));
    messageCount++;
  });
